...
const categoryLabels = ['Todos', 'Ficção', 'Romance', 'Biografias', 'Fantasia', 'Tecnologia'];
...
const Home: React.FC = () => {
  ...
  const getDistanceLabel = (index: number) => {
    ...
    return `${(index * 0.9 + 1.2).toFixed(1)} km de você`;
  };
      ...
      <div className="min-h-screen bg-[#F7FAF8] pb-28 md:pb-12">
        <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-6 px-4 py-4 sm:px-6 md:gap-8 md:px-8 md:py-6">
          <header className="flex flex-col gap-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative w-full sm:max-w-xl">
                ...
                <input
                  type="text"
                  placeholder="Pesquisar livros, autores ou gêneros..."
          ...
          <section className="rounded-[32px] border border-black/6 bg-white px-5 py-6 shadow-[0_24px_70px_rgba(15,23,42,0.06)] sm:px-7 sm:py-8">
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_420px] lg:items-start">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <p className="max-w-2xl text-sm leading-7 text-text-muted sm:text-base">
                    Descubra livros disponíveis, encontre leitores próximos e acompanhe as melhores oportunidades de troca em um único lugar.
                  </p>
                </div>

                <Link
                  to="/explore"
                  className="group block overflow-hidden rounded-[28px] border border-black/6 bg-[#F4FBF6] shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Acervo aberto', value: `${books.length}+`, text: 'títulos disponíveis para explorar', icon: 'auto_stories' },
                  { label: 'Perto de você', value: `${nearbyBooks.length}+`, text: 'livros sugeridos por proximidade', icon: 'near_me' },
                  { label: 'Categorias', value: categoryLabels.length, text: 'frentes para começar sua busca', icon: 'category' },
                ].map((stat) => (
          ...
          <section className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.24em] text-primary">Catálogo aberto</p>
                <h2 className="mt-2 text-2xl font-black tracking-tight text-text-main">Livros disponíveis agora</h2>
              </div>
                    ...
                    <div className="space-y-3 p-4">
                      ...
                      <div className="flex items-center justify-between pt-1">
                        ...
                        <span className="rounded-full bg-[#F4F8F5] px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-primary">
                          {locationStatus === 'granted' ? getDistanceLabel(index).replace(' de você', '') : 'Catálogo'}
                        </span>
                      </div>
                    </div>
                  </Link>
